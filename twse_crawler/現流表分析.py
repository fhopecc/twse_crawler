...
def 分析現金流(股票) -> "pandas.Series":
    ...
    歷史季度財報["滾動四季其他項目"] = 歷史季度財報.其他項目.fillna(0).rolling(window=4).sum()
    歷史季度財報["營業現金流及淨利比"] = (
    ...
    # 2. 計算維度一：絕對水位（中位數）
    營業現金流及淨利比中位數 = float(分析數據["營業現金流及淨利比"].median())
    自由現金流及營收比中位數 = float(分析數據["自由現金流佔營收比"].median())

    # 3. 計算維度二：方向趨勢（OLS 斜率）
    X = np.arange(1, 總季度數 + 1)
    X_加常數項 = sm.add_constant(X)

    營業現金流及淨利比_Y = 分析數據["營業現金流及淨利比"].values
    營業現金流及淨利比迴歸 = sm.OLS(營業現金流及淨利比_Y, X_加常數項).fit()
    營業現金流及淨利比趨勢 = float(營業現金流及淨利比迴歸.params[1]) if len(營業現金流及淨利比迴歸.params) > 1 else 0.0

    自由現金流及營收比_Y = 分析數據["自由現金流佔營收比"].values
    自由現金流及營收比迴歸 = sm.OLS(自由現金流及營收比_Y, X_加常數項).fit()
    自由現金流及營收比趨勢 = (
        float(自由現金流及營收比迴歸.params[1])
        if len(自由現金流及營收比迴歸.params) > 1
        else 0.0
    )
    ...
    if 營業現金流及淨利比中位數 >= 1.0:
        營業現金流及淨利比水位得分 = 2.0
    elif 營業現金流及淨利比中位數 >= 0.8:
        營業現金流及淨利比水位得分 = 1.0
    else:
        營業現金流及淨利比水位得分 = -3.0

    if 自由現金流及營收比中位數 >= 0.10:
        自由現金流水位得分 = 2.0
    elif 自由現金流及營收比中位數 >= 0.05:
        自由現金流水位得分 = 1.0
    else:
        自由現金流水位得分 = -3.0
    ...
    if 營業現金流及淨利比趨勢 > 0.02:
        營業現金流及淨利比趨勢調整 = 1 
    elif 營業現金流及淨利比趨勢 < -0.02:
        if 營業現金流及淨利比中位數 < 1.0:
            營業現金流及淨利比趨勢調整 = -2

    if 自由現金流及營收比趨勢 > 0.02:
        自由現金流趨勢調整 = 1 
    elif 自由現金流及營收比趨勢 < -0.02:
        if 自由現金流及營收比中位數 < 0.10:
            自由現金流趨勢調整 = -2
    ...
    # 四、致命重罰（死亡背離檢查）
    最近其他項目與淨利比 = 歷史季度財報.其他項目與淨利比.iloc[-1]
    if 淨利趨勢 > 0.02 and 營業現金流趨勢 < -0.02:
        from twse_crawler.營收動能分析 import 分析營收動能
        公司型態 = 分析營收動能(股票).營收模式
        if 公司型態 == '崑鼎型公司' and 最近其他項目與淨利比 > 0.5:
            m.append('淨利大幅增加，惟營業現金流卻減少，主要係調整無現金流動之服務特許權建造收入')
        else:
            m.append('淨利大幅增加，惟營業現金流卻減少扣8分')
            致命死亡背離重罰 = -8
    ...
    分析報告 = pd.Series({
        "分數": 現金流最終得分,
        "評語": 評語,
        "長期營業現金流及淨利比中位數": 營業現金流及淨利比中位數,
        "長期自由現金流及營收比中位數": 自由現金流及營收比中位數,
        "營業現金流及淨利比趨勢值": 營業現金流及淨利比趨勢,
        "自由現金流及營收比趨勢值": 自由現金流及營收比趨勢,
        "營業現金流變異係數": 營業現金流變異係數,
        "自由現金流轉負季度比例": 自由現金流轉負比例,
        "淨利趨勢值": 淨利趨勢,
        "營業現金流趨勢值": 營業現金流趨勢,
        "分析數據季度數": 總季度數,
        "營業現金流及淨利比水位得分": 營業現金流及淨利比水位得分,
        "自由現金流水位得分": 自由現金流水位得分,
        "營業現金流及淨利比趨勢調整": 營業現金流及淨利比趨勢調整,
        "自由現金流趨勢調整": 自由現金流趨勢調整,
        "營業現金流波動調整": 營業現金流波動調整,
        "自由現金流失血調整": 自由現金流失血調整,
        "致命死亡背離重罰": 致命死亡背離重罰,
        "最近其他項目與淨利比": 最近其他項目與淨利比
    })
    return 分析報告
