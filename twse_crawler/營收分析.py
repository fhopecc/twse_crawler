...
def 取預測盈餘說明(年度每股盈餘, 前年至次年各季數據):
    ...
    if 'ds' in 前年至次年各季數據.columns:
        df_quarterly = 前年至次年各季數據.set_index('ds')
    else:
        df_quarterly = 前年至次年各季數據
        df_quarterly.index = 前年至次年各季數據.index.to_timestamp(how='end').normalize()
...
def 取歷月營收表(股票=None):
    ...
    if 股票:
        股票代號 = 查股票代號(股票)
        h = 取歷月營收表()
        h = h.query('公司代號==@股票代號').copy()
        h['營收'] = h['營業收入-當月營收']
        h['營收月份索引'] = h.營收月份
        h = h.set_index('營收月份索引')
        h = h.sort_index()
        return h
    if 今日.day < 15:
...
@通知執行時間
def 預測前年至次年每股盈餘舊(股票, 歷月營收表=None):
    '''
...
def 以營收預測次年每股盈餘(股票, 歷月營收表=None):
    ...
    預估方法說明 = (f'以{預測營收結果.預估方法說明}'
         f'，輸入{表達預估方法(預測營利結果, "營利")}'
         f'，與{表達預估方法(預測業外損益結果, "業外損益")}'
         f'，加總之稅前損益'
         f'，扣除最高稅率20％之營所稅之損益'
...
@通知執行時間
def 以營收預測稅前淨利及次年每股盈餘(股票, 歷月營收表=None):
    '''
    一、運用指定「股票」歷月營收表預測前年至次年每股盈餘。
    二、預測結果：前年至次年每股盈餘、預估說明、預估方法說明
    三、相上相容：預測說明、營收趨勢起日、營收數據頻率、
    ...
    # 以營收預測稅前淨利
    from twse_crawler.預估次年底 import 依外部季數據預估次年底數值
    預測稅前淨利結果 = 依外部季數據預估次年底數值(歷季損益表.稅前淨利.dropna()
                                             ,歷季損益表[['營收']]
                                             ,預估目標='稅前淨利', 單位='元')
    預測稅前淨利方法說明 = 表達預估方法(預測稅前淨利結果, '稅前淨利')
    預測稅前淨利 = 預測稅前淨利結果.預估各季值
    歷季損益表['稅前淨利'] = 歷季損益表.稅前淨利.fillna(預測稅前淨利)
    ...
    from twse_crawler.預估次年底 import 移除重覆時間詞
    預估方法說明 = (f'以{預測營收結果.預估方法說明}'
         f'，輸入{表達預估方法(預測稅前淨利結果, "稅前淨利")}'
         f'，扣除最高稅率20％之營所稅之損益'
         f'，再除以{取最簡約數(股數)}股之每股盈餘'
         )
    預估說明 = (f'{預測營收結果.預估說明}'
                f'，{表達預估說明(預測稅前淨利結果, "稅前淨利")}'
                f'，{取預測盈餘說明(年度每股盈餘, 前年至次年各季數據)}'
                )
    預測結果 = pd.Series({'前年至次年每股盈餘': pd.Series(年度每股盈餘)
                         ,'預測說明':預估說明+"。"+預估方法說明
