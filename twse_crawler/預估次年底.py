def 表達預估方法(預估結果, 預估目標='業外損益', 單位='元', 時間單位='季'):
    ...
    try:
        預估誤差範圍說明 = f'所得以過去{m.回測資料數:,}{時間單位}訓練，誤差{m.誤差率:,.2%}'
...
def 表達預估說明(預估結果, 預估目標='毛利率', 時間單位='季'):
    ...
    return f'{表達期間(預估結果.最近歷史值時間)}{預估目標}同比{取增減百分比(預估結果.最近歷史值同比)}，預估次{時間單位}同比{取增減百分比(預估結果.首期預估值同比)}，誤差{預估結果.誤差率:,.0%}'
...
def 預估至次年底每季值乙式(
    ...
    # =====================================================================
    # 📊 6. 🛠️ 重新實地回測：計算最終確定的驗證指標 (rmse 與 mape)
    # =====================================================================
    最終回測預測值 = []
    for i in range(回測季數):
        當前終點 = len(y_原始) - 回測季數 + i
        ...
        try:
            模型 = ThetaModel(y_訓練, period=4, deseasonalize=True)
            模型擬合 = 模型.fit()
            預測值 = 模型擬合.forecast(1).values[0]
            最終回測預測值.append(預測值)
        except:
            最終回測預測值.append(np.nan)

    最終回測_陣列 = np.array(最終回測預測值)
    真實值 = y_原始.iloc[-回測季數:]

    # 檢查回測值是否有效，若有效則計算指標，否則安全填充為 np.nan
    if len(最終回測_陣列) == 回測季數 and np.isfinite(最終回測_陣列).all():
        mape = float(np.mean(np.abs((真實值 - 最終回測_陣列) / (真實值 + 1e-5))))
        rmse = float(np.sqrt(mean_squared_error(真實值, 最終回測_陣列)))
    else:
        mape = np.nan
        rmse = np.nan

    # =====================================================================
    # 7. 擬合最終 Theta 預估模型（使用最優記憶視窗）
    # =====================================================================
    最終訓練起點 = max(0, len(y_原始) - 最佳視窗)
    y_最終訓練 = y_原始.iloc[最終訓練起點:]
