...
def 表達預估方法(預估結果, 預估目標='業外損益', 單位='元', 時間單位='季'):
    ...
    return (
        f'{訓練資料說明}'
        f'所得回測{m.回測資料數:,}{時間單位}之誤差範圍為{取最簡約數(m.rmse)}{單位}'
        f'，比率{m.mape:,.2%}'
...
def 預估至次年底每月值(
    ...
    預測結果 = pd.Series({
        ...
        "rmse": rmse_值,
        "mape": mape_值,
        "模型名稱": f"滾動{最佳視窗}個月歷史值，{最佳模式}模式之ThetaModel"
    })
    return 預測結果
...
def 以外部月數據預估次年底各月值(
    ...
    if len(外部數據名稱) == 0:
        raise ValueError("傳入的 外部月數據表 沒有任何特徵欄位！")
        
    # 設定月度模型的滾動回測月數（通常設定為 12 個月，即完整回測一整年）
    回測月數 = 12
