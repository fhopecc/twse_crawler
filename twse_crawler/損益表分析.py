...
def 是否適合以歷史淨利預測每股盈餘(股票):
    from twse_crawler.營收動能分析 import 分析崑鼎型公司
    try:
        r = 分析崑鼎型公司(股票)
        if r.是否為崑鼎型公司:
            return True
...
# @增加股票分析函數依資料時間更新快取功能(損益表分析結果快取檔, ['營收月份', '財報季度'])
@通知執行時間
def 以淨利預測次年每股盈餘(股票):
    '''
    ...
    if not 是否適合以歷史淨利預測每股盈餘(股票):
        raise ValueError(f'{公司簡稱}非一零四型公司，不適合逕以淨利時間序列預測每股盈餘')
    ...
    預估淨利結果 = 預估至次年底每季值丙式(歷季損益表.淨利.dropna())
    預測淨利 = 預估淨利結果.預估各季值
    future_index = 預測淨利.index[預測淨利.index > 歷季損益表.index.max()]
    ...
    預估方法說明 = (f'{表達預估說明丙(預估淨利結果, "淨利")}'
         f'，扣除最高稅率20％之營所稅之損益'
         f'，再除以{取最簡約數(股數)}股之每股盈餘'
         )
    預估說明 = [f'【營收預測結果】{預測營收結果.預估說明}'
               ,f'【淨利預測結果】{表達預估說明丙(預估淨利結果, "淨利")}'
               ,f'【每股盈餘預測結果】{取預測盈餘說明(年度每股盈餘, 前年至次年各季數據)}，誤差{預估淨利結果.誤差率:,.0%}'
               ]
    預估說明 = '；'.join(預估說明)
