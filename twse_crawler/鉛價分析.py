...
def 以鉛價預測次年底毛利率(股票):
    ...
    預估鉛季價 = 預測次年底鉛價()
    鉛價模型準確度 = 表達預估方法(預估鉛季價)
    預估鉛季價 = 預估鉛季價.預估季價[['季均價', '季起迄價差']]
    df = 取財報彙總表(股票).set_index('財報日期').to_period('Q')
    r = 依外部季數據預估次年底數值(df.毛利率, 預估鉛季價)
    r['預估方法說明'] = (f'以{鉛價模型準確度}模型預測之鉛價'
                           f'，輸入以{表達預測準確度(r)}模型預測毛利率'
                          )
    r['預估說明'] = f'以鉛價預測次年底毛利率預估'
    return r
...
def 以鉛價預測次年每股盈餘(股票, 歷月營收表=None):
    ...
    二、預測結果：前年至次年每股盈餘、預測說明、預估方法說明、營收趨勢起日、營收數據頻率、
