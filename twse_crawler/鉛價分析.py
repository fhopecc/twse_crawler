...
def 預測次年底鉛價(回測日數=0):
    ...
    ps = 取鉛價().現價
    d = ps.index.max()
    d -= pd.Timedelta(days=回測日數)
    ps = ps[ps.index <= d]
    r = 預估至次年底工作日值丙式(ps)
    return r

@通知執行時間
def 以鉛價預測次年底毛利率(股票, 回測季數=0):
    """
    一、預估結果項目：預估各季值、
                      模型名稱、誤差率、歷史值數量、預估值數量、回測資料數、
...
def 以鉛價預測次年每股盈餘(股票, 歷月營收表=None):
    ...
    每股盈餘預估說明 = f'【盈餘預測】{取預測盈餘說明(年度每股盈餘, 前年至次年各季數據)}'
    預估說明 = (f'【營收預測】{表達預估說明丙(預測營收結果)}'
                f'【鉛價預測結果】{表達預估說明丙(鉛價預測結果, '鉛價', '日')}'
                f'【毛利率預測】{預測毛利率結果.預估說明}'
...
@通知執行時間
def 以鉛價預測次年淨利(股票, 歷月營收表=None, 回測季數=0):
    '''
    一、回測季數係將指定股票目前資料往回季數之資料作為預測歷史資料，
        最近回測資料作為評估誤差率使用。
    '''
    from twse_crawler.股票基本資料分析 import 查股票簡稱, 查股票代號, 取股票基本資料彙總表
    from twse_crawler.預估至次年底每季值 import 表達預估方法丙, 表達預估說明丙
    ...
    # 預測毛利率及毛利
    預測毛利率結果 = 以鉛價預測次年底毛利率(股票, 回測季數=回測季數)
    鉛價預測結果 = 預測毛利率結果.鉛價預測結果
    預測毛利率 = 預測毛利率結果.預估各季值
    歷季損益表['毛利率'] = 歷季損益表.毛利率.fillna(預測毛利率)
    歷季損益表['毛利'] = 歷季損益表.毛利.fillna(預測營收*預測毛利率)
    ...
    預測業外損益結果 = 以匯率預測次年底業外損益(股票, 回測季數=回測季數)
    預測業外損益 = 預測業外損益結果.預估各季值
    歷季損益表['業外損益'] = 歷季損益表.業外損益.fillna(預測業外損益)
