...
def 取股利表(股票=None):
    ...
    try:
        ...
        logger.error(f'抓取公司股利分派公告資料彙總表({今日:%y%m%d})失敗，係發生{e}')
...
def 依損益表預測每股盈餘(股票):
    ...
    '''
    from twse_crawler.損益表分析 import 取前年至次年各季損益表, 取損益表
...
def 依歷年股利預測股利(股票):
    ...
    from twse_crawler.損益表分析 import 取前年至次年各季損益表, 取損益表, cache as acache
    from twse_crawler.自結損益 import 預測前年至次年周期數據
    from zhongwen.時 import 取正式民國日期
    from zhongwen.數 import 取最簡約數
...
def 預測股利(股票, 歷年股利=None):
    ...
    一、如有自結損益，則據以預估前年至次年股利；
        ...
        悉闕，則告以數據不足。
    二、預測結果：前年至次年股利、誤差率、預估詳情、
                  預測股利說明、除息交易日、除權交易日、現金股利發放日
    三、除息交易日為最近一次除息交易日。
    ...
    for 預測每股盈餘 in [以鉛價預測次年每股盈餘
        ...
        try:
            ...
            try:
                ...
                誤差率 = 1
            break
        except (數據不足, 無法預估盈餘, ValueError) as e:
            continue
    else: # 依歷年股利預測股利
