...
def 評定基礎分數(股票) -> "pandas.Series":
    ...
    r = 評定現金週轉天數分數(股票)
    現金週轉天數得分, 現金週轉天數評語 = r.分數, r.評語
    r = 評定負債結構分數(股票)
    負債扣分項, 負債評語 = r.分數, r.評語
    ...
    if 營收動能結果.營收模式!='類崑鼎公司' and 無形資產佔淨值比 > 0.15:
        資產風險扣分 += 2.0
        觸發無形資產警訊 = True
    ...
    if 資產風險扣分 > 0:
        評語字串.append(f"資產純度風險扣除 {資產風險扣分:,.1f} 分")
    if 護城河近期是否結構性崩塌:
        評語字串.append( "🚨【暴雷：最新季度毛利與營利雙殺，護城河發生結構性崩塌！】")
    if 觸發無形資產警訊:
        評語字串.append("⚠️【警訊：無形資產佔淨值偏高，警惕商譽減損風險】")
    if 觸發表外投資警訊:
        評語字串.append("🚨【資產黑洞：權益法長期投資佔比過高，警惕隱藏表外虧損實體！】")
    if 營收動能結果.是否喪失營收動能:
        總基礎得分 = 0
        評語字串.append(f"【喪失營收動能，主要係{營收動能結果.喪失營收動能原因}】")
