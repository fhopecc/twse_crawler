...
def 評級股票(股票, 告示例外=True, 例外為空=False) -> "pandas.Series":
    '''
    一、股票評級結果：報酬率、預測報酬率說明、誤差率、預估詳情、分數、評語、近一季重大訊息、。
    '''
...
def 取股票評級彙總表():
    ...
    二、欄位：公司代號(主鍵)、公司簡稱、產業類別、報酬率、預測報酬率說明、誤差率、預估詳情、
              分數、評語、前年至次年股利。
    三、預估詳情係各細項預測結果及誤差率說明。
    '''
...
def 評定基礎分數(股票) -> "pandas.Series":
    ...
    """
    import pandas as pd
    from twse_crawler.權益報酬率分析 import 評定權益報酬率分數
    from twse_crawler.營利率分析 import 評定營利率分數
    ...
    r = 評定權益報酬率分數(股票)
    報酬率得分, 報酬率評語 = r.分數, r.評語
    r = 評定營利率分數(股票)
    營利率得分, 營利率評語 = r.分數, r.評語
    r = 評定現金週轉天數分數(股票)
    現金週轉天數得分, 現金週轉天數評語 = r.分數, r.評語
    r = 分析現金流(股票)
    現金流得分, 現金流評語 = r.分數, r.評語
    r = 評定負債結構分數(股票)
    ...
    # ==========================================
    總基礎得分 = (
        報酬率得分 + 營利率得分 + 現金週轉天數得分 + 現金流得分 - 負債扣分項 - 資產風險扣分
    )
    總基礎得分 = max(0, 總基礎得分)
    # 建立動態評語
    評語字串 = [
