...
def 預測次年底美元匯率():
    ...
    r = m[["預估值", "預估季均值"]]
    r['預估方法說明'] = (
        f'依{取正式民國日期(m.最近歷史值日期)}前{m.歷史值數量:,}日美元匯率訓練'
        f'{m.模型名稱}模型，'
        f'其回測{m.回測資料數:,}日之變動範圍{m.rmse:,.2f}元，比率{m.mape:,.2%}，'
        f'預估至{m.最後預估值日期.year-1911:,}年底之{m.預估值數量:,}日美元匯率'
        )
    return r
...
def 以匯率預測次年底業外損益(股票):
    ...
    r['預估方法說明'] = f'輸入{預估季均匯率結果.預測方法說明}之匯率，運用{r.模型準確度說明}'
    return r
