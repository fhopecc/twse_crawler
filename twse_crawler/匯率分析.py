...
def 以匯率預測次年底業外損益(股票, 回測季數=0):
    """
    一、預估結果項目：預估各季值、
                      模型名稱、誤差率、歷史值數量、預估值數量、回測資料數、
                      最佳訓練資料數、趨勢、近期影響權重、自變數影響權重
                      最近歷史值時間、最後預估值時間、
                      最近歷史值同比、首期預估值同比、預估匯率結果。
    二、預估各季值為歷史加各季預估值。
    """
    ...
    預估季均匯率結果 = 預測次年底美元匯率(回測日數=回測季數*90)
    ...
    歷季匯率 = 預估季均匯率[預估季均匯率.index <= df.index.max()]
    預測匯率 = 預估季均匯率[預估季均匯率.index > df.index.max()]
    r = 以單元迴歸預估至次年底每季值(預估季均匯率, df.業外損益,  預測匯率)
    r['預估匯率結果'] = 預估季均匯率結果
    return r
