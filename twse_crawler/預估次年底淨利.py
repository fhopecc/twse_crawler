...
def 預估次年底淨利(股票, 重新評估模型=False):
    '''
    一、輸出：前年至次年每股盈餘、預估說明、誤差率、預估方法說明、
              最近財報季度、最近營收月份、預測說明
    二、誤差率係 WMAPE。
    ...
    try:
        ...
        if 股票 in 毛利受鉛價影響者:
            鉛價預測次年淨利結果 = 以鉛價預測次年淨利(股票)
        else:
            鉛價預測次年淨利結果 = None
        if not 鉛價預測次年淨利結果 is None:
            r = pd.DataFrame([
                 {'模型': 西塔預估結果.模型名稱, '誤差率':西塔預估結果.誤差率}
                ,{'模型': 月營收逐步推估淨利結果.模型名稱, '誤差率':月營收逐步推估淨利結果.誤差率}
                ,{'模型': 鉛價預測次年淨利結果.模型名稱, '誤差率':鉛價預測次年淨利結果.誤差率}
                ])
        else:
            r = pd.DataFrame([
                             {'模型': 西塔預估結果.模型名稱, '誤差率':西塔預估結果.誤差率}
        ...
        各股預估模型誤差率明細檔[股票] = r
        min_row = r.loc[r.誤差率.idxmin()]
        for m in [西塔預估結果, 月營收逐步推估淨利結果, 鉛價預測次年淨利結果]:
            if (not m is None) and m.模型名稱 == min_row.模型:
                m['模型評估結果'] = r
                if hasattr(m, '各項預估說明'):
                    m['各項預估說明'] = f'{m.各項預估說明}'
                else:
...
def 以淨利預測次年每股盈餘(股票):
    ...
    二、預測結果：前年至次年每股盈餘、預估說明、預估方法說明、每股盈餘預估說明。
    ...
    每股盈餘預測結果 = f'【每股盈餘預測結果】{取預測盈餘說明(年度每股盈餘, 前年至次年各季數據)}'
