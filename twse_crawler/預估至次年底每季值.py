def 表達預估說明丙(預估結果, 預估目標='淨利', 時間單位='季', 自變數名稱=None):
    ...
    if 'Theta' in 預估結果.模型名稱:
        if '季節' in 預估結果.模型名稱:
            ...
            模型個別預測項目 = f'，所得趨勢{"向上" if 預估結果.趨勢 > 0 else "向下"}'
        from zhongwen.表 import 表示
        try:
            模型個別預測項目 += f'，近期影響{預估結果.近期影響權重:,.2%}'
        except AttributeError as e:
            print(f'{預估結果}發生{e}')
        模型個別預測項目 += f'，係過去{預估結果.最佳訓練資料數}{時間單位}{預估目標}訓練'
        模型個別預測項目 += f'，回測{預估結果.回測資料數}{時間單位}估計'
    elif 'OLS' in 預估結果.模型名稱:
        模型個別預測項目 = f'，所得趨勢{"向上" if 預估結果.趨勢 > 0 else "向下"}'
        模型個別預測項目 += f'，{自變數名稱}可解釋{預估結果.自變數影響權重:,.2%}，估計'
    else:
        模型個別預測項目 = ''
    ...
    m = f'{表達期間(預估結果.最近歷史值時間)}{預估目標}同比{取增減百分比(預估結果.最近歷史值同比)}，{預估次期同比}'
    if 預估結果.誤差率:
        m += f'*誤差{預估結果.誤差率:,.0%}*'
    else:
        m += '*無法估計誤差率*'
