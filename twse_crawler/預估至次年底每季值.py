...
def 取預估至次年底每季值模型(
    ...
    三、最佳模型：係回測 4 季之評估指標最小 Theta 之訓練資料數及是否去季節情形。
                 若 Theta 模型 WMAPE 比無腦模型高，則改採較優之無腦模型。
    ...
    if best_wmape < 無腦基線最優誤差:
        ...
        較無腦模型改善率 = (無腦基線最優誤差 - best_wmape) / 無腦基線最優誤差 if 無腦基線最優誤差 > 0 else np.nan
    else:
        # 無腦模型勝出 (WMAPE 較低或相等)
        最佳訓練資料數 = len(y) - 回測季數
        y_best_train = y
...
def 預估至次年底每季值丙式(
    ...
    # 4. 外推未來預測值至次年底（依模型類型區分）
    if "Theta" in 模型名稱 and 最終模型擬合 is not None:
        預估季_陣列 = 最終模型擬合.forecast(外推步數)
        趨勢 = 最終模型擬合.params.b0
        近期影響權重 = 最終模型擬合.params.alpha
    elif 模型名稱 == "以上期值預測":
        # Naive：重複最後一季的數值
        預估季_陣列 = pd.Series(y.iloc[-1], index=未來季時軸)
        趨勢 = 0.0
        近期影響權重 = 1.0
    elif 模型名稱 == "以去年同期值預測":
        # SNaive：重複去年同期的數值
        循環數 = (外推步數 + 3) // 4
        週期數值 = y.iloc[-4:].values
        SNaive預測陣列 = np.tile(週期數值, 循環數)[:外推步數]
        預估季_陣列 = pd.Series(SNaive預測陣列, index=未來季時軸)
        趨勢 = 0.0
        近期影響權重 = 0.0
    else:
        預估季_陣列 = pd.Series(y.iloc[-1], index=未來季時軸)
